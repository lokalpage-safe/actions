...
class ShortLivedTokenClient {
    ...
    async fetchToken(serverUrl: string, accessKey: HostnameAccessKey, expiry: string): Promise<HostnameAccessKey> {
        ...
        while (attempts < this.maxRetries) {
            try {
                const response = await this.httpc.post(`${serverUrl}/api/auth/token${queryParams}`, '', headers)
...
class DevelocityAccessCredentials {
    static readonly accessKeyRegexp = /^(\w+=\w+)(;\w+=\w+)*$/
    ...
    raw(): string {
        return this.keys
            .map(k => `${k.hostname}${DevelocityAccessCredentials.keyDelimiter}${k.key}`)
